...
if [[ ($# -gt 1 || `pwd | rev | cut -d'/' -f1 | rev` != "rockfish") ]]
then
    echo "need to invoke from the root 'rockfish' directory"
    exit 1
fi
...
LDFLAGS="-X github.com/Reidmcc/rockfish/cmd.version=$VERSION_STRING -X github.com/Reidmcc/rockfish/cmd.gitBranch=$GIT_BRANCH -X github.com/Reidmcc/rockfish/cmd.gitHash=$GIT_HASH -X github.com/Reidmcc/rockfish/cmd.buildDate=$DATE"
...
then
    ...
    # generate outfile
    OUTFILE=bin/rockfish$EXTENSION
    mkdir -p bin
...
do
    ...
    # archive
    ARCHIVE_FILENAME=rockfish-$VERSION-$GOOS-$GOARCH$GOARM.tar
    cd $ARCHIVE_DIR
...
echo -n "cleaning up rockfish folder: $ARCHIVE_DIR_SOURCE/ ... "
cd $ARCHIVE_DIR
